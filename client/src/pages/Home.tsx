...
function isFurnitureCategory(value: string): value is FurnitureCategory {
  return [
    ...
    "Decor",
    "Custom",
  ].includes(value as FurnitureCategory);
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (dbPlans.length > 0) {
      const convertedPlans: FloorPlan[] = dbPlans.map((dbPlan) => ({
        ...
        rooms: parseJsonArray(dbPlan.roomsJson, []),
        furniture: parseJsonArray(dbPlan.furnitureJson, []),
  ...
  useEffect(() => {
    if (dbCustomFurniture.length > 0) {
      const converted: FurnitureTemplate[] = dbCustomFurniture.map((item) => ({
        id: item.id,
        name: item.name,
        category: isFurnitureCategory(item.category) ? item.category : "Custom",
  ...
  }, [plans, isAuthenticated, saveFloorPlanMutation]);
  ...
  }, [customFurniture, isAuthenticated, saveCustomFurnitureMutation]);
